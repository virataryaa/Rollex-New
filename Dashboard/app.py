...
rollex_tab, corr_tab = st.tabs(["Rollex", "Correlation Matrix"])

# =============================================================================
# TAB 1 — ROLLEX (all existing content unchanged)
# =============================================================================
with rollex_tab:
    ...
    # SECTION 2 — Daily Return Distribution
...
with corr_tab:
    ...
    all_data_corr = load_all()

    # Build aligned price dataframe across all commodities
    px_dict = {}
    for comm in AVAILABLE:
        if comm in all_data_corr:
            s = all_data_corr[comm]["rollex_px"].dropna()
            px_dict[comm] = s
    px_all = pd.DataFrame(px_dict).sort_index()

    cm_min_d = px_all.index.min().date()
    cm_max_d = px_all.index.max().date()
    cm_default_start = max(cm_min_d, (pd.Timestamp(cm_max_d) - pd.DateOffset(years=3)).date())
    ...
    with ctrl_a:
        cm_range = st.slider(
            "Date range",
            min_value=cm_min_d, max_value=cm_max_d,
            value=(cm_default_start, cm_max_d),
            key="cm_dates",
        )
    ...
    with ctrl_b:
        ret_mode = st.radio(
            "Return window",
            options=["Daily", "Rolling 10d", "Rolling 20d", "Custom"],
            horizontal=False,
            key="cm_ret_mode",
        )
    ...
    with ctrl_c:
        custom_window = st.number_input(
            "Custom window (days)",
            min_value=1, max_value=252, value=30,
            key="cm_custom_win",
            disabled=(ret_mode != "Custom"),
        )

    window_map = {"Daily": 1, "Rolling 10d": 10, "Rolling 20d": 20, "Custom": int(custom_window)}
    n_days = window_map[ret_mode]

    # ── Compute returns & correlation ─────────────────────────────────────────
    cm_start, cm_end = cm_range
    px_fil = px_all.loc[str(cm_start):str(cm_end)]
    rets_matrix = px_fil.pct_change(n_days).dropna(how="all")

    corr_matrix = rets_matrix.corr()
    labels = [COMM_NAMES[c].split(" — ")[0] for c in corr_matrix.columns]

    # ── Heatmap ───────────────────────────────────────────────────────────────
    z_vals  = corr_matrix.values
    n       = len(labels)
    text_vals = [[f"<b>{z_vals[i][j]:.2f}</b>" if i != j else "<b>1.00</b>"
    ...
    fig_corr = go.Figure(go.Heatmap(
        ...
        xgap=2, ygap=2,
        colorbar=dict(
            title=dict(text="r", font=dict(size=10, color="#1d1d1f")),
            tickfont=dict(size=9, color="#1d1d1f"),
    ...
    fig_corr.update_layout(
        ...
        xaxis=dict(tickfont=dict(size=10, color="#1d1d1f"), side="bottom", showgrid=False),
        yaxis=dict(tickfont=dict(size=10, color="#1d1d1f"), showgrid=False, autorange="reversed"),
        **_D,
    )

    mode_label = ret_mode if ret_mode != "Custom" else f"Custom {n_days}d"
    st.markdown(
        lbl(f"Correlation Matrix — {mode_label} Returns  ·  {cm_start.strftime('%d %b %Y')} → {cm_end.strftime('%d %b %Y')}"),
        unsafe_allow_html=True,
    )
    ...
    # ── Ranked pairs table ────────────────────────────────────────────────────
    st.markdown(lbl("Ranked Pairs — Strongest to Weakest Correlation"), unsafe_allow_html=True)

    pair_rows = []
    comms_list = list(corr_matrix.columns)
    for i in range(len(comms_list)):
        for j in range(i + 1, len(comms_list)):
            ca, cb = comms_list[i], comms_list[j]
    ...
    pairs_df = pd.DataFrame(pair_rows).sort_values("_r", ascending=False).reset_index(drop=True)

    r_colors   = [("#1a6b1a" if r >= 0.7 else "#e8a020" if r >= 0.4 else
                   "#8b0000" if r <= -0.4 else "#1d1d1f") for r in pairs_df["_r"]]
    str_colors = [("#1a6b1a" if s == "Strong" else "#e8a020" if s == "Moderate" else "#888888")
                  for s in pairs_df["Strength"]]

    fig_pairs = go.Figure(go.Table(
        columnwidth=[120, 80, 80],
        header=dict(
            values=["Pair", "Correlation", "Strength"],
            fill_color=NAVY, font=dict(color="white", size=10),
            align="center", height=28,
        ),
        cells=dict(
            values=[pairs_df["Pair"], pairs_df["Correlation"], pairs_df["Strength"]],
            fill_color=[["white" if i % 2 == 0 else "#f5f5f7" for i in range(len(pairs_df))]],
            font=dict(
                color=[[("#1d1d1f")] * len(pairs_df), r_colors, str_colors],
                size=10,
            ),
            align="center", height=24,
        ),
    ))
    fig_pairs.update_layout(height=max(300, len(pairs_df) * 26 + 60),
                            margin=dict(t=0, b=0, l=0, r=0), **_D)

    _, tbl_col, _ = st.columns([1, 2, 1])
    with tbl_col:
        st.plotly_chart(fig_pairs, use_container_width=True)

    st.caption(
        f"Based on {mode_label} returns  ·  "
        f"{cm_start.strftime('%d %b %Y')} → {cm_end.strftime('%d %b %Y')}  ·  "
        f"{len(rets_matrix)} observations"
    )
